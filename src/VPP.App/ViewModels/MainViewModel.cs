...
public partial class MainViewModel : ObservableObject
{
    ...
    [RelayCommand]
    private void DeleteMeasurement(MeasurementResult result)
    {
        if (result == null) return;

        MeasurementHistory.Remove(result);

        // If the deleted item was the current one, clear it
        if (CurrentMeasurementResult == result)
        {
            CurrentMeasurementResult = null;
            MeasurementStatusMessage = "Measurement deleted";
        }

        StatusMessage = $"{result.Name} deleted from history";
    }
