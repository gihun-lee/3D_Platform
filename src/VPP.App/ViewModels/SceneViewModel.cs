...
public partial class SceneViewModel : ObservableObject
{
    ...
    {
        // Update PointCloudColor: white when depth enabled to allow per-vertex colors to show, LightGray when disabled
        PointCloudColor = value ? Colors.White : Colors.LightGray;
    ...
    public void UpdatePointCloud(List<PointCloudData> clouds, bool fitCamera)
    {
        ...
        // Ensure PointCloudColor is correct for current visualization mode
        PointCloudColor = IsDepthVisualizationEnabled ? Colors.White : Colors.LightGray;
    ...
    private PointCloudData ApplyDepthColors(PointCloudData cloud, float globalMinZ, float globalMaxZ)
    {
        ...
        var coloredCloud = new PointCloudData
        {
            ...
            // IMPORTANT: Create a COPY of the cache, not a reference!
            // The cache is reused across ApplyDepthColors calls, so assigning the reference
            // would cause all clouds to share the same color list (with the last cloud's data)
            Colors = new List<System.Numerics.Vector3>(_depthColorCache)
        };
